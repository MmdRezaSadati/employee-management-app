...
const Profile: FC<{ id: string }> = ({ id }) => {
  const data = employees.find((item) => String(item.id) === id);
  return (
    <div className="relative box-border w-72 rounded-md bg-white p-5 text-sm leading-5 text-gray-900 dark:bg-black-light/5 dark:text-gray-200">
      <div className="mb-5 flex h-10 w-60 items-center justify-between">
      ...
      <div className="mb-5 w-60">
        <div className="flex h-36 w-60 flex-col items-center justify-center">
          <Image
            width={96}
            height={96}
        ...
        <ul className="mt-7 flex w-60 list-none items-center justify-center gap-4">
